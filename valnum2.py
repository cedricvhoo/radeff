...
# Geometry
Matm = np.array([[1,1,1],[1,0,1],[1,1,1]])  # Geometry matrix 
Lxim = np.ones((1,3))                       # Vector with lengths of subplates in x direction
Lyim = np.ones((3,1))                       # Vector with lengths of subplates in y direction
...
xB = np.hstack((np.arange(0,Lxi+dx,dx),np.arange(2*Lxi,Lx+dx,dx)))  # x coordinates of part B (2 times Lxi * Lyi)
yB = np.arange(Lyi+dy,2*Lyi,dy)                                     # y coordinates of part B (2 times Lxi * Lyi)
xC = np.arange(0,Lx+dx,dx)                  # x coordinates of part C (Lx * Lyi)
yC = np.arange(2*Lyi,Ly+dy,dy)              # y coordinates of part C (Lx * Lyi)
...
XC = np.transpose(np.tile(xC, (1, len(yC))))                        # Repeat xB for all elements of yB
YC = np.reshape(np.tile(yC[:, np.newaxis], (1, len(xC))), (-1, 1))  # Repeat yB for all elements of xB

X1 = np.vstack((XA,XB,XC))                  # Combine x coordinates of parts A, B, and C
Y1 = np.vstack((YA,YB,YC))                  # Combine y coordinates of parts A, B, and C
